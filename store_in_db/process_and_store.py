...
def main() -> None:
    ...
    earnings_file = ROOT_DIR / "earning_dates" / "earning_dates.json"

    # ── symbol list ────────────────────────────────────────────────────────
    with open(ROOT_DIR / "nse_fno_scripts.json") as fh:
        fno = json.load(fh)
    symbols = [x["symbol"]
               for grp in ("index_futures", "individual_securities")
               for x in fno[grp]]
    ...
    if earnings_file.exists():
        with open(earnings_file) as fh:
            for rec in json.load(fh):
                if rec.get("event_type") != "stock_results":
                    continue
                sym, d = rec.get("trading_symbol"), rec.get("date")
                try:
    ...
    for csv in sorted(bhavcopy_dir.glob("*.csv")):
        ...
        for sym in symbols:
            ...
            if all(float(fut_row[c]) > 0 for c in ("OPEN", "HIGH", "LOW", "CLOSE")):
            ...
            if len(rows) >= WINDOW:
                yz = compute_yz_rolling_vol(
                    pd.DataFrame(rows[-MAX_LOOKBACK:]),
            ...
            def _iv(p, T, call=True):
                return implied_volatility_bisection(p, spot, strike, T, r_dec,
                                                    is_call=call) * 100
            ...
            payload["ce"] = {
                ...
                "low":   float(ce30["LOW"]),
                "ivp": None, "ivr": None,          # filled later
                "greeks": black_scholes_greeks(
                    spot, strike, T30, r_dec, iv30_ce/100.0, is_call=True),
            }
            ...
            payload["pe"] = {
                ...
                "ivp": None, "ivr": None,
                "greeks": black_scholes_greeks(
                    spot, strike, T30, r_dec, iv30_pe/100.0, is_call=False),
            }
