...
def main() -> None:
    ...
    for sym in SYMBOLS:
        ...
        try:
            ...
            spot_price_map[sym] = {
                d: v["underlying_price"]
                for d, v in ts.items()
                if "underlying_price" in v
            }
        except Exception:
            continue
    ...
    for sym in SYMBOLS:
        ...
        for fp in bhav_files:
            ...
            rec["ce"] = dict(
                iv_30=iv_ce30, iv_60=_iv(ce60, T60, True),
                iv_90=_iv(ce90, T90, True),
            ...
            rec["pe"] = dict(
                iv_30=iv_pe30, iv_60=_iv(pe60, T60, False),
                iv_90=_iv(pe90, T90, False),
        ...
        if daily_ohlc:
            ...
            for idx, rv in yz.items():
                k = idx.strftime("%d-%b-%Y")
                if k in time_map and np.isfinite(rv):
                    time_map[k]["rv_yz"] = rv
        ...
        def _iv_stats(rows: List[tuple[datetime, float]]):
            if len(rows) < WINDOW:
                return {}
            df = pd.DataFrame(rows, columns=["Date", "iv"]).dropna()
        ...
        for k, rec in time_map.items():
            ...
            if dt in ce_stat:
                rec["ce"]["ivp"], rec["ce"]["ivr"] = \
                    ce_stat[dt]["ivp"], ce_stat[dt]["ivr"]
            if dt in pe_stat:
